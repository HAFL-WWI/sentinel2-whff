############################################################################
# Predict forest type (coniferous/mixed/deciduous) using multi-temporal sentinel-2 data and 
...
setwd("//mnt/cephfs/data/HAFL/WWI-Sentinel-2/Data/TBk_Sementina_Gudo/")
...
SHAPE.NAME = "random_sample_plots_mv"
# NEW
SP.PATH = paste(getwd(), "/", SHAPE.NAME, ".shp", sep="")
...
for(i in 1:length(stackList$stack)) {
  ...
  # pred$S2REP = 705+35*((((pred$B7+pred$B4)/2)-pred$B5)/(pred$B6-pred$B5))
  pred$NDI45 = (pred$B5-pred$B4)/(pred$B5+pred$B4)
  # pred$MTCI = (pred$B6-pred$B5)/(pred$B5-pred$B4)
