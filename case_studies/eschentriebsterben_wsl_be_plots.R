############################################################################
# Eschentriebsterben Daten WSL: Boxplots and data mining
#
...
# load field data
sp_field = readOGR("//mnt/cephfs/data/HAFL/WWI-Sentinel-2/Data/Eschentriebsterben/Daten_WSL_BE/Eschentriebsterben_WSL_wgs84.shp")
sp_field = gBuffer(sp_field, byid=T, width=25)
cro_means = as.data.frame(aggregate(sp_field$CRO, list(sp_field$SITE), mean))
colnames(cro_means) = c("SITE", "CRO_MEAN")
coords = unique(coordinates(sp_field))
coordinates(cro_means) = coords

# plot overview
plotRGB(s2_l1c_20170510[[4:2]], stretch="lin", axes=T)
plot(cro_means, add=T, col="red")

# TODO use calc function and multi-core (https://www.rdocumentation.org/packages/raster/versions/2.6-7/topics/cluster)
# NDVI
...
# extract
df = as.data.frame(extract(stk, cro_means, fun=mean, sp=T, na.rm=T))

# clean up
df = df[complete.cases(df), ]

# build to groups
df$group = rep("low", nrow(df))
df$group[df$CRO_MEAN >= 2.5] = "high"
df$group = as.factor(df$group)

# plot
par(mfrow = c(3, 2),oma=c(0,0,2,0))
plot(df$NDVI.20170410 ~ df$group, main="Sentinel-2 Bild 10. April 2017", ylab="NDVI", xlab="CRO mean (<2.5, >2.5")
plot(df$NDVI.20170430 ~ df$group, main="Sentinel-2 Bild 30. April 2017", ylab="NDVI", xlab="CRO mean (<2.5, >2.5")
plot(df$NDVI.20170510 ~ df$group, main="Sentinel-2 Bild 5. Mai 2017", ylab="NDVI", xlab="CRO mean (<2.5, >2.5")
plot(df$NDVI.20170619 ~ df$group, main="Sentinel-2 Bild 6. Juni 2017", ylab="NDVI", xlab="CRO mean (<2.5, >2.5")
plot(df$NDVI.20170818 ~ df$group, main="Sentinel-2 Bild 18. August 2017", ylab="NDVI", xlab="CRO mean (<2.5, >2.5")
plot(df$NDVI.20171012 ~ df$group, main="Sentinel-2 Bild 12. Oktober 2017", ylab="NDVI", xlab="CRO mean (<2.5, >2.5")
title("Eschentriebsterben - WSL BE Data", outer=TRUE)
par(mfrow = c(1, 1))

# write CSV
write.csv(df, "//mnt/cephfs/data/HAFL/WWI-Sentinel-2/Data/Eschentriebsterben/Daten_WSL_BE/s2_l1c_spectral_data_cro_means.csv")
